...
is_valid_python() {
    ...
    if [ -x "$bin" ] || command -v "$bin" >/dev/null 2>&1; then
        "$bin" -c "import sys; assert (3, 10) <= sys.version_info < (3, 15); import venv" >/dev/null 2>&1
        return $?
    fi
    return 1
}

# Find the best available Python interpreter on macOS
find_python() {
    ...
    if [ -d "$pyenv_root/versions" ]; then
        for pattern in "3.12*" "3.11*" "3.13*" "3.10*" "*"; do
            for ver_dir in "$pyenv_root/versions"/$pattern; do
                if [ -d "$ver_dir" ]; then
    ...
    # 4. Check Homebrew standard locations (Apple Silicon & Intel)
    local brew_prefixes=("/opt/homebrew" "/usr/local")
    for bp in "${brew_prefixes[@]}"; do
    ...
    for fw_ver in "3.12" "3.11" "3.13" "3.10" "Current"; do
        local fw_bin="/Library/Frameworks/Python.framework/Versions/$fw_ver/bin/python3"
    ...
    for c_py in "${conda_candidates[@]}"; do
...
if [ ! -d "$VENV_DIR" ]; then
    ...
    if [ -z "$PYTHON_BIN" ]; then
        echo "[mcp-speak] Error: No compatible Python interpreter (>= 3.10, < 3.15) found on this Mac." >&2
